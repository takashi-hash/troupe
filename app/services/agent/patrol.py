...
def patrol(
    ...
    for id in states.ids_in("Failed") + states.ids_in("FinishedPendingRecheck"):
        ...
        assessment = Assessment(finding=finding, reason=_fact_reason(job, material))
    ...
    for id in states.ids_in("InProgress"):
        ...
        assessment = Assessment(
            finding=f"自力では進めない（直近の止まった理由: {last}）",
            reason=(
                f"止まった理由 {len(material.fall_reasons)}件・"
                f"前に出した成果{'あり' if material.previous_result is not None else 'なし'}・"
                f"やり直し {job.retried}回"
            ),
        )
        assessments.put(id, assessment)
