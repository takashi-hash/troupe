...
設計: 設計/人に見えるもの.md §2。
| 訪問の詳細 | 訪問の識別子・日付・患者の要約（患者の行の全欄）・**未使用の下書き**
（本文と識別子）・**署名済みの記録の列**・担当の名簿。**押せるのは署名と1回だけ休むの2つ** |
...
class UnusedDraft(BaseModel):
    """未使用の下書き — 署名の初期値になる提案。識別子は使用の印を入れる先。"""
...
class VisitView(BaseModel):
    ...
    status: str
    patient: PatientRow
    #: 未使用の下書き（新しい順）。無ければ空——白紙から書いて署名してよい。
    drafts: tuple[UnusedDraft, ...]
