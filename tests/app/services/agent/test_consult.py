...
) -> tuple[Refusal | None, 成果置き場の偽物, 根拠置き場の偽物]:
    成果 = 成果置き場の偽物()
    根拠 = 根拠置き場の偽物()
    断り = consult(
        帳簿, 材料読み or 材料読みの偽物(), 源, llm, 成果, 根拠, 固定時計(), 仕事.id,
        by=働き手,
    )
    return 断り, 成果, 根拠
...
def test_質問へ抜ける() -> None:
    ...
    断り, _, _ = _進める(帳簿, 仕事, 源の偽物(材料), llm)
    assert 断り is None
    後 = 帳簿.jobs[仕事.id]
    assert isinstance(後.state, AwaitingAnswer)
    尋ね = next(e for e in 帳簿.events if isinstance(e, QuestionAsked))
    assert 尋ね.body == "どの環境の依存を見ますか"  # 本文は出来事が完載する——正本
...
def test_成果と根拠を積む() -> None:
    ...
    断り, 成果, 根拠 = _進める(帳簿, 仕事, 源の偽物(材料, 引用), llm)
    assert 断り is None
    後 = 帳簿.jobs[仕事.id]
...
def test_引用が取れなければ根拠なしで出す() -> None:
    ...
    帳簿, 仕事 = _実行中の仕事()
    llm = LLMの偽物(Reply(mark=Mark.RESULT, body="2026-W34 の依存は42件、更新3件"))
    断り, _, 根拠 = _進める(帳簿, 仕事, 源の偽物(材料, Unreadable(reason="源が閉じた")), llm)
    assert 断り is None
    後 = 帳簿.jobs[仕事.id]
...
def test_源が読めなければ落ちる() -> None:
    ...
    断り, _, _ = _進める(帳簿, 仕事, 源の偽物(Unreadable(reason="源に接続できませんでした")), llm)
    assert 断り is None
    後 = 帳簿.jobs[仕事.id]
...
def test_別のAIの名乗りは断られる() -> None:
    ...
    断り = consult(
        帳簿, 材料読みの偽物(), 源の偽物(材料), llm, 成果置き場の偽物(),
        根拠置き場の偽物(), 固定時計(), 仕事.id, by=Agent(name="二号"),
    )
