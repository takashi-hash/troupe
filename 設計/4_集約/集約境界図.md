...
**2026-08-21（段C）に分けた**。仕立て（`new_job`）が生成の規則を持ち、
再構成（`reconstitute`）は**型の形だけ**を見る。

そのうえで**古い帳簿は捨ててよい**と決めた——注入し直せばボードと業務ルールは戻るから。
だめなのは黙って壊れること。帳簿は自分の形の番号を刻み、番号が違えば
「読めない、入れ直せ」と名乗って開かない（`StaleLedger`）。
