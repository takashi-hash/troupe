...
- **Signing is one transaction.** On the visit screen the doctor edits the AI's draft (S/O/A/P prefilled) and signs — **the signer is the seat itself** (a clinician's seat on the register; there is no picking another name). One EMR transaction stores the signed note and marks the visit done. The draft itself is never written to — a draft is addressed to one visit (patient × date, unique), so "was it used" is derived from the signed record's existence, and every record still traces back through its visit-addressed draft to the AI job that wrote it. From that moment a **database trigger refuses any edit or delete** of the note.
